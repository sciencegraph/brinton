#' Presents a specific graphic determined by its name.
#'
...
#' @param diagram Character. Specific graphic to be presented.
#' @param output Character. The type of output can be a 'html' file, the
#' 'plots pane' or the 'console'.
#'
#' @return A html file that includes a or the code to produce it.
#' @export
#'
#' @examples
#' \dontrun{showplot(iris, "Petal.Length", "point graph")}
...
)
{
  ...
  if(lubridate::is.Date(unlist(data[, vars])) == TRUE & (diagram %in% datetime_v) == FALSE)
...
ggplot(",
  ...
  as.character(substitute(vars)),
  ")) +
  geom_bin2d(aes(y=1), binwidth = c(1, 1), fill='black') +
  ", theme_detail_y)
  }
